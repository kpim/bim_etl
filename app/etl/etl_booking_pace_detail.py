...
def init_sp_fload_booking_pace_detail():
    ...
    for property in etl_opera.PROPERTIES:
        ...
        NULL AS [STATUS], ROOM_CAT AS R_TYPE, RTC AS R_CHARGE,
...
def fload():
    print(f"Full load data into table booking_pace_detail")
    conn = get_connection()
    cursor = conn.cursor()
...
def iload():
    print(f"Incremental load data into table booking_pace_detail")
    conn = get_connection()
    cursor = conn.cursor()
