...
def init_sp_fload_booking_pace_report():
    ...
    BEGIN TRY;
        ...
        FROM
        (SELECT *, 
            CASE WHEN ISNULL(DATEDIFF(DAY, CREATE_DATE, ARRIVAL), 0) >= 0 THEN ISNULL(DATEDIFF(DAY, CREATE_DATE, ARRIVAL), 0) >= 0 ELSE 0 END AS WINDOW_DAYS
            FROM dbo.booking_pace_detail
        ) d LEFT JOIN dbo.window w ON d.WINDOW_DAYS >= w.[FROM] AND (d.WINDOW_DAYS <= w.[TO] OR w.[TO] IS NULL)
...
def init_sp_iload_booking_pace_report():
    ...
    BEGIN TRY;
        ...
        LEFT JOIN dbo.window w ON d.WINDOW_DAYS >= w.[FROM] AND (d.WINDOW_DAYS <= w.[TO] OR w.[TO] IS NULL)
